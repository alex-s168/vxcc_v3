...
void opt_join_compute(SsaView view, SsaBlock *block) {
    ...
    for (size_t i = 0; i < block->ops_len; i ++) {
        ...
        while (j --> 0) {
            ...
            if (prev->id != op->id || !irop_is_pure(op) || prev->params_len != op->params_len || prev->outs_len == 0)
                continue;
            ...
            irop_add_param_s(op, SSA_NAME_VALUE, (SsaValue) { .type = SSA_VAL_VAR, .var = prev->outs[0].var });
